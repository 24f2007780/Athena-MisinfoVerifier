...
from Claim_Verification.prompts import (
...
async def evaluate_evidence_node(state: ClaimVerifierState) -> dict:
    ...
    if not response:
        ...
        # Optional entailment check (content-focused) when borderline
        try:
            enable_entail = os.getenv("ENABLE_ENTAILMENT_CHECK", "false").lower() == "true"
        except Exception:
            enable_entail = False
        if enable_entail and result in (VerificationResult.SUPPORTED, VerificationResult.REFUTED):
            try:
                top_text = "\n\n".join(s.text for s in sources[:2]) if sources else ""
                entail_system = "You are checking whether evidence entails or contradicts the claim. Answer with 'entails', 'contradicts', or 'uncertain' and one short reason."
                entail_human = f"Claim: {claim.claim_text}\nEvidence:\n{top_text}"
                entail_msg = [("system", entail_system), ("human", entail_human)]
                entail_llm = get_llm()
                entail_resp = await entail_llm.ainvoke(entail_msg)
                entail_text = str(entail_resp)
                if result == VerificationResult.SUPPORTED and "contradict" in entail_text.lower():
                    verdict.result = VerificationResult.REFUTED
                    verdict.reasoning = verdict.reasoning + " (entailment check flagged contradiction)"
                if result == VerificationResult.REFUTED and "entail" in entail_text.lower():
                    verdict.result = VerificationResult.SUPPORTED
                    verdict.reasoning = verdict.reasoning + " (entailment check flagged entailment)"
            except Exception:
                pass

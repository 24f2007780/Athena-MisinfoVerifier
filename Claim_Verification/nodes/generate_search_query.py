...
async def generate_search_query_node(
    ...
    # If user provided seed queries in state, honor them in order first
    if all_queries:
        # Use next provided query if there are any pre-seeded remaining
        seeded_remaining = [q for q in all_queries if q]
        if iteration_count < len(seeded_remaining):
            next_query = seeded_remaining[iteration_count]
            logger.info(f"Using provided query (iteration {iteration_count + 1}): {next_query}")
            return {"query": next_query, "all_queries": all_queries}

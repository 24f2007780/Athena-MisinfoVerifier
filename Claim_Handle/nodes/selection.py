...
from Claim_Handle.prompts import HUMAN_PROMPT, SELECTION_SYSTEM_PROMPT
from Claim_Handle.schemas import ContextualSentence, SelectedContent, State
...
async def _single_selection_attempt(
    ...
    # Lightweight factual-density scoring (fallback-only, no external deps)
    try:
        import re
        proper_nouns = len(re.findall(r"\b[A-Z][a-z]+\b", processed))
        digits = len(re.findall(r"\d", processed))
        has_trigger = 1 if ("according to" in processed.lower() or "report" in processed.lower()) else 0
        score = proper_nouns + digits + has_trigger
        # Drop very low-scoring sentences
        if score == 0 and len(processed.split()) < 8:
            log_kv(logger, logging.DEBUG, "selection_filter", dropped=True, reason="low_factual_density")
            return False, None
    except Exception:
        pass

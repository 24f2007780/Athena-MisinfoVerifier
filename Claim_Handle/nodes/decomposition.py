...
from Claim_Handle.schemas import DisambiguatedContent, PotentialClaim, State
...
async def _decomposition_stage(
    ...
    # Filter out fragments: require simple subject-verb shape heuristics
    filtered_claims: List[str] = []
    for c in claims_texts:
        tokens = c.split()
        if len(tokens) < 6:
            continue
    ...
    potential_claims = [
        ...
        for claim_text in filtered_claims
    ]
